...
-- Si agrupamos, vemos el total: De 6 ciudades, 
en las cuales tiene un determinado numero de filas,
si podemos ver, tenemos que la ciudad #1 (Bogota) tiene 87 filas,
la ciudad #2 (Leticia) tiene 92 filas,
la ciudad #3 (Cartagena) tiene 91 filas,
la ciudad #4 (Medellin) tiene 82 filas,
la ciudad #5 (Cali) tiene 72 filas,
la ciudad #6 (Barranquilla) tiene 76 filas;
SELECT CiudadID,
    COUNT(*) AS Filas
FROM FactVentas
GROUP BY CiudadID;
...
en la ciudad 6 que es Barranquilla tenemos un:
costo total = 15960.0
costo promedio = 210.0
...
SELECT c.Ciudad, p.Producto, p.Categoria, ROUND(SUM(f.Precio_Venta * f.Cantidad * (1 - f.Descuento_Pct)), 2) AS Venta_Neta
FROM FactVentas f
INNER JOIN DimProducto p ON f.ProductoID = p.ProductoID
INNER JOIN DimCiudad c ON f.CiudadID = c.CiudadID
WHERE c.Ciudad = 'Leticia'
